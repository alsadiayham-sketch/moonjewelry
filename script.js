...
let cart = normalizeCartItems(JSON.parse(localStorage.getItem('moon_cart') || '[]'), normalizeProducts(DEFAULT_PRODUCTS));
let deliveryMethod = localStorage.getItem('moon_delivery_method') || 'pickup';
...
function subscribeToStoreData() {
    ...
    // Load first 6 products fast, then load remaining
    db.collection('products').orderBy('id').limit(6).get().then(function (snapshot) {
        ...
        // Fall back to full onSnapshot if initial get fails
...
function saveCart() {
    localStorage.setItem('moon_cart', JSON.stringify(normalizeCartItems(cart, products.length ? products : normalizeProducts(DEFAULT_PRODUCTS))));
}

function setDeliveryMethod(method) {
    deliveryMethod = method;
    localStorage.setItem('moon_delivery_method', method);
